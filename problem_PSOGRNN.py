...
from GRNNandPSO import distance_mat, Gauss, sum_layer, output_layer, optimize_sigma
...
def train_and_test(features):
    ...
    # 使用粒子群优化算法获取最优的光滑因子σ值
    best_sigma = optimize_sigma(X_train, X_test, y_train, y_test)

    # 使用GRNN模型进行回归分析（使用最优的σ值）
    Euclidean_D = distance_mat(X_train, X_test)
    Gauss_mat = Gauss(Euclidean_D, best_sigma)
...
df_results.to_excel('results_pso_GRNN.xlsx', index=False)
print(df_results)