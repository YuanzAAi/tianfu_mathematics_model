...
def train_and_test(features):
    ...
    y_train = y_train.ravel()  # 将y_train转换为一维数组
    y_test = y_test.ravel()  # 将y_test转换为一维数组
    clf = MLPClassifier(max_iter=2000,random_state=0)  # 创建一个多层感知机分类器
...
df_results.to_excel('resultsMLP.xlsx', index=False)
